...
def get_dataset(sequence_length, batch_size):
    ...
    data_test_df = pd.read_pickle(fd_km + '/data_test_v1').reset_index().iloc[:,1:] # The folder has to be changed to the location of the training set
...
model_path = model_directory + '/' + 'TCN_Model_adaptiveLR_1'

#model.load_weights(model_directory + '/TCN_Model_adaptiveLR_1_weights_epoch_7_val_accuracy_0.8480284810066223.h5') # activate this option to use weights from hyperparameter tuning (not done in paper for TCNs)
